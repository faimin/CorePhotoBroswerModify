...
- (void)vcPrepare
{
	...
	//间距
	_scrollViewRightMarginC.constant = -margin;
}
...
- (void)pagesPrepare
{
	...
	CGFloat widthEachPage = frame.size.width + margin;
...
- (void)showWithPage:(NSUInteger)page
{
	...
	if ([self.visiblePhotoItemViewDictM objectForKey:@(page)] != nil) {
		return;
	}
...
- (IBAction)rightBtnClick:(id)sender
{
	...
	if ([itemModel read]) { //已经保存过本地
		[CoreSVP showSVPWithType:CoreSVPTypeInfo Msg:@"图片已保存" duration:1.0f allowEdit:NO beginBlock:nil completeBlock:nil];
	}
	else {
		//展示提示框
